...
```{r}
# Read the CSV data set into a data frame
data <- read_csv(here::here("week8", "southpark-sdc.csv"))
...
The stu-id, SSN, name and dob will be removed soon from the data set as they are direct identifiers.
...
file <- read.csv(here::here("week8", fname))
...
```{r}
# The treshold depends on the size of the data set and the access control (conservative number for large surveys are 0.04)
...
sdcInitial <- groupAndRename(obj = sdcInitial, var = c("age"),
                             before = c("8","9","10"),
                             after = c("8-10"))
sdcInitial <- groupAndRename(obj = sdcInitial, var = c("age"),
                             before = c("11","12","13"),
...
Only a tiny improvement compared to the original data set. Let's try something else.
...
Alright! We managed lower the risk of identification from 81% to about 10% and now we have 0 observations violating 5-anonymity! We can tell our clients we used some recoding, but supression via k-anonymity was necessary to improve the privacy level of this data set.
...
sdcInitical <- createNewID(sdcInitial, newID = "ID", withinVar = "stu_id")
```
